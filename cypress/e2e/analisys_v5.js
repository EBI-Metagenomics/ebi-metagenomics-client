...
describe('Analysis V5', () => {
  ...
  context('Overview tab', () => {
    ...
    it('Sections should be expandable', () => {
      ...
      cy.get('#tab-overview details:nth-child(1)').should(
          'not.have.attr',
          'open'
      );
      cy.get('#tab-overview details:nth-child(1) > summary').click();
      ...
      cy.get('#tab-overview details:nth-child(2)').should(
          'not.have.attr',
          'open',
          'closed'
      );
  ...
  context.skip('QC tab', () => {
    ...
    it('Should load [Number of of contigs per QC] chart', () => {
      ...
      cy.get(
          '#qc-step-chart .highcharts-series-1 > rect.highcharts-point'
      ).should('have.length', 6); // there is one empty

      cy.get(
          '#qc-step-chart g.highcharts-xaxis-labels > text:nth-child(1)'
      ).should('contain', 'Initial contigs');
      cy.get(
          '#qc-step-chart g.highcharts-xaxis-labels > text:nth-child(2)'
      ).should('contain', 'Trimming');
      cy.get(
          '#qc-step-chart g.highcharts-xaxis-labels > text:nth-child(3)'
      ).should('contain', 'Length filtering');
      cy.get(
          '#qc-step-chart g.highcharts-xaxis-labels > text:nth-child(4)'
      ).should('contain', 'Ambiguous base filtering');
      cy.get(
          '#qc-step-chart g.highcharts-xaxis-labels > text:nth-child(5)'
      ...
      const tooltipValues = [
        ...
        {
          series: 4,
          tooltip: 'Ambiguous base filtering● Contigs remaining: 148 992'
        },
        {
          series: 5,
    ...
    it('Should load [Contigs length hist] chart', () => {
      openWaitAndChangeTab(pageUrl, `Analysis ${analysisId}`, 'qc');
      cy.get('#reads-length-hist .highcharts-series-group').should(
          'be.visible'
      );
    });
    ...
    it('Should load [Contigs length bar] chart', () => {
      openWaitAndChangeTab(pageUrl, `Analysis ${analysisId}`, 'qc');
      cy.get(
        '#reads-length-barchart g.highcharts-xaxis-labels > text:nth-child(1)'
      ).should('contain', 'Minimum');
      cy.get(
          '#reads-length-barchart g.highcharts-xaxis-labels > text:nth-child(2)'
      ).should('contain', 'Average');
      cy.get(
          '#reads-length-barchart g.highcharts-xaxis-labels > text:nth-child(3)'
    ...
    it('Should load [Contigs GC distribution] chart', () => {
      openWaitAndChangeTab(pageUrl, `Analysis ${analysisId}`, 'qc');
      cy.get(
        '#reads-gc-barchart g.highcharts-xaxis-labels > text:nth-child(1)'
    ...
    it('Should load [Nucleotide position histogram] chart', () => {
      openWaitAndChangeTab(pageUrl, `Analysis ${analysisId}`, 'qc');
      const series =
        '#nucleotide-chart .highcharts-series-group .highcharts-area-series';
  ...
  context('Functional tab', () => {
    ...
    const tabs = ['InterPro', 'GO Terms', 'Pfam', 'KO'];
