...
;(function($){
	...
	tsColSel = ts.columnSelector = {
		...
		queryAll   : '@media only all { [columns] { display: none; ***REMOVED*** ***REMOVED*** ',
		queryBreak : '@media all and (min-width: [size]) { [columns] { display: table-cell; ***REMOVED*** ***REMOVED*** ',
		...
		init: function(table, c, wo) {
			...
			***REMOVED***
			...
			colSel = c.selector = { $container : $(wo.columnSelector_container || '<div>') ***REMOVED***;
		...
		***REMOVED***,
		...
		setupSelector: function(c, wo) {
			var index, name, $header, priority, col, colId, $el,
				...
				state = useStorage ? ts.storage( c.table, 'tablesorter-columnSelector-auto') : {***REMOVED***;
			...
			for ( index = 0; index < c.columns; index++ ) {
				...
				***REMOVED***
				...
				if ($container.length) {
					...
					***REMOVED***
					colSel.$wrapper[colId] = $(wo.columnSelector_layout.replace(/\{name\***REMOVED***/g, name)).appendTo($container);
		...
		setupBreakpoints: function(c, wo) {
			...
			***REMOVED***
			...
			if (colSel.$container.length) {
				...
				if (wo.columnSelector_mediaquery) {
					colSel.$auto = $( wo.columnSelector_layout.replace(/\{name\***REMOVED***/g, wo.columnSelector_mediaqueryName) ).prependTo(colSel.$container);
					colSel.$auto
		...
		***REMOVED***,
		updateCols: function(c, wo) {
			...
			***REMOVED***
			if (colSel.$style) {
				colSel.$style.prop('disabled', false).text( styles.length ? styles.join(',') + ' { display: none; ***REMOVED***' : '' );
			***REMOVED***
			tsColSel.saveValues( c, wo );
			tsColSel.adjustColspans( c, wo );
			c.$table.triggerHandler(wo.columnSelector_updated);
		...
		adjustColspans: function(c, wo) {
			...
			for ( index = 0; index < len; index++ ) {
				...
				if ( span > 1 ) {
					...
					***REMOVED***
				***REMOVED*** else if ( typeof colSel.states[ col ] !== 'undefined' && colSel.states[ col ] !== null ) {
		...
		***REMOVED***,
		...
		saveValues : function( c, wo ) {
			if ( wo.columnSelector_saveColumns && ts.storage ) {
				var colSel = c.selector;
				ts.storage( c.$table[0], 'tablesorter-columnSelector-auto', { auto : colSel.auto ***REMOVED*** );
	...
	***REMOVED***;
	...
	ts.addWidget({
		...
		options: {
			...
			columnSelector_columns : {***REMOVED***,
			// remember selected columns
			columnSelector_saveColumns: true,

			// container layout
			columnSelector_layout : '<label><input type="checkbox">{name***REMOVED***</label>',
			// layout customizer callback called for each column
			// function($cell, name, column){ return name || $cell.html(); ***REMOVED***
		...
		***REMOVED***,
		remove: function(table, c, wo, refreshing) {
			var csel = c.selector;
			if ( refreshing || !csel ) { return; ***REMOVED***
			if ( csel ) { csel.$container.empty(); ***REMOVED***
			if ( csel.$popup ) { csel.$popup.empty(); ***REMOVED***
			csel.$style.remove();
