...
;(function($){
	...
	tsff = ts.filterFormatter = $.extend( {***REMOVED***, ts.filterFormatter, {
		...
		uiSpinner: function($cell, indx, spinnerDef) {
			...
			***REMOVED***, spinnerDef ),
			...
			$input = $('<input class="filter" type="hidden">')
				...
				.bind('change' + c.namespace + 'filter', function(){
					updateSpinner({ value: this.value, delayed: false ***REMOVED***);
				***REMOVED***),
			$shcell = [],
			...
			***REMOVED***;
			...
			o.create = function(event, ui) {
				...
				if (typeof o.oldcreate === 'function') { o.oldcreate(event, ui); ***REMOVED***
			***REMOVED***;
			o.spin  = function(event, ui) {
				updateSpinner(ui);
				if (typeof o.oldspin === 'function') { o.oldspin(event, ui); ***REMOVED***
			***REMOVED***;
			if (o.addToggle) {
			...
			***REMOVED***
			...
			c.$table.bind('filterFomatterUpdate', function(){
				...
				updateSpinner({ value: val ***REMOVED***, true);
		...
		uiSlider: function($cell, indx, sliderDef) {
			...
			***REMOVED***, sliderDef ),
			...
			$input = $('<input class="filter" type="hidden">')
				...
				// hidden filter update namespace trigger by filter widget
				.bind('change' + c.namespace + 'filter', function(){
					updateSlider({ value: this.value ***REMOVED***);
				***REMOVED***),
			$shcell = [],
			...
			***REMOVED***;
			o.slide  = function(event, ui) {
				updateSlider(ui);
				if (typeof o.oldslide === 'function') { o.oldslide(event, ui); ***REMOVED***
			***REMOVED***;
			...
			c.$table.bind('filterFomatterUpdate', function(){
				...
				updateSlider({ value: val ***REMOVED***, false);
				ts.filter.formatterUpdated($cell, indx);
			***REMOVED***);
			...
			if (o.compare) {
				...
				tsff.addCompare($cell, indx, o);
				$cell.find(compareSelect).bind('change', function(){
					updateSlider({ value: $cell.find('.slider').slider('value') ***REMOVED***);
				***REMOVED***);
			***REMOVED***
			...
			c.$table.bind('filterReset', function(){
				...
				***REMOVED***
				setTimeout(function(){
					updateSlider({ value: o.value ***REMOVED***);
				***REMOVED***, 0);
			***REMOVED***);
		...
		uiRange: function($cell, indx, rangeDef) {
			...
			***REMOVED***, rangeDef ),
			...
			getRange = function(){
				var val = $input.val(),
					v = val.split(' - ');
				if (val === '') { v = [ o.min, o.max ]; ***REMOVED***
				if (v && v[1]) {
					updateUiRange({ values: v, delay: false ***REMOVED***, true);
				***REMOVED***
			***REMOVED***,
		...
		uiDateCompare: function($cell, indx, defDate) {
			...
			***REMOVED***, defDate),
			...
			date1Compare = function(notrigger) {
				...
				if (getdate === '') { notrigger = false; ***REMOVED***
			...
			***REMOVED***;
			...
			o.onClose = function( selectedDate, ui ) {
				date1Compare();
				if (typeof o.oldonClose === 'function') { o.oldonClose(selectedDate, ui); ***REMOVED***
			***REMOVED***;
			$date.datepicker(o);
			...
			***REMOVED***);
			...
			c.$table.bind('filterFomatterUpdate', function(){
				...
				***REMOVED*** else {
					...
					// differeniate 1388556000000 from 1/1/2014 using \d{5***REMOVED*** regex
					num = num !== '' ? /\d{5***REMOVED***/g.test(num) ? new Date(Number(num)) : num || '' : '';
				***REMOVED***
			...
			***REMOVED***;
			...
			c.$table.bind('filterFomatterUpdate', function(){
				...
				***REMOVED***
				...
				// differeniate 1388556000000 from 1/1/2014 using \d{5***REMOVED*** regex
				from = from !== '' ? /\d{5***REMOVED***/g.test(from) ? new Date(Number(from)) : from || '' : '';
				to = to !== '' ? /\d{5***REMOVED***/g.test(to) ? new Date(Number(to)) : to || '' : '';
