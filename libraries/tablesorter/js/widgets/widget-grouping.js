	...
	tsg = ts.grouping = {
		...
		***REMOVED***,
		findWeek : function( wo, day ) {
			...
			***REMOVED*** else if ( !$.isEmptyObject( wo.group_week ) ) {
				// CLDR returns { sun: "Sun", mon: "Mon", tue: "Tue", wed: "Wed", thu: "Thu", ... ***REMOVED***
		...
		***REMOVED***,
		findTime : function( wo, time, part ) {
			var suffix,
				// CLDR returns { am: "AM", pm: "PM", ... ***REMOVED***
		...
		***REMOVED***,
		...
		update : function(table) {
			if ($.isEmptyObject(table.config.cache)) { return; ***REMOVED***
			var c = table.config,
				wo = c.widgetOptions,
			...
			***REMOVED***
			if (column >= 0 && column < c.columns && !c.$headerIndexed[column].hasClass('group-false')) {
				...
				wo.group_collapsedGroups = {***REMOVED***;

				data.column = column;
				// group class finds 'group-{word/separator/letter/number/date/false***REMOVED***-{optional:#/year/month/day/week/time***REMOVED***'
				data.groupClass = (c.$headerIndexed[column].attr('class') || '').match(/(group-\w+(-\w+)?)/g);
				// grouping = [ 'group', '{word/separator/letter/number/date/false***REMOVED***', '{#/year/month/day/week/time***REMOVED***' ]
		...
		processHeaders : function( c, wo, data ) {
			...
			for ( index = 0; index < len; index++ ) {
				...
				if ( wo.group_count || $.isFunction( wo.group_callback ) ) {
					...
					if ( $label.length ) {
						if ( wo.group_count ) {
							$label.html( wo.group_count.toString().replace( /\{num\***REMOVED***/g, $rows.length ) );
						***REMOVED***
				...
				***REMOVED*** else if ( wo.group_collapsed && wo.group_collapsible ) {
		...
		***REMOVED***,
		saveCurrentGrouping : function( c, wo, data ) {
			...
			if (wo.group_collapsible && wo.group_saveGroups) {
				wo.group_collapsedGroups = ts.storage && ts.storage( c.table, 'tablesorter-groups' ) || {***REMOVED***;
		...
		bindEvents : function(table, c, wo){
			if (wo.group_collapsible) {
				...
				c.$table.on('click toggleGroup keyup', 'tr.group-header', function(event){
					...
					if (event.type === 'keyup' && event.which !== 13) { return; ***REMOVED***
					...
					***REMOVED***
					...
					if (wo.group_saveGroups && ts.storage) {
						...
						***REMOVED*** else if (wo.group_collapsedGroup) {
	...
	***REMOVED***;
	...
	ts.addWidget({
		...
		options: {
			...
			group_count       : ' ({num***REMOVED***)', // if not false, the '{num***REMOVED***' string is replaced with the number of rows in the group
			group_separator   : '-',  // group name separator; used when group-separator-# class is used.
			group_formatter   : null, // function(txt, column, table, c, wo) { return txt; ***REMOVED***
			group_callback    : null, // function($cell, $rows, column, table){***REMOVED***, callback allowing modification of the group header labels
			...
			group_dateString  : function(date) { return date.toLocaleString(); ***REMOVED***
		***REMOVED***,
