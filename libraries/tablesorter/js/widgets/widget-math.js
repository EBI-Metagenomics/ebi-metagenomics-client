...
;( function( $ ) {
		...
		processText : function( c, $cell ) {
			...
			if ( /</.test( prefix ) ) {
				...
				tmp = $( '<div>' + prefix + '</div>' ).text()
					.replace(/\{content\***REMOVED***/g, '').trim();
				txt = txt.replace( tmp, '' );
			***REMOVED***
				...
				while ( index >= 0 ) {
					...
					***REMOVED***);
					...
					if ( ( ( hasFilter || !$tr.hasClass( filtered ) ) &&
						...
						if ( mathAbove ) {
							index = 0;
						***REMOVED*** else if ( $t.length ) {
							arry[ arry.length ] = math.processText( c, $t );
						***REMOVED***
					***REMOVED***
					index--;
				***REMOVED***
			***REMOVED*** else if ( type === 'below' ) {
				len = $rows.length;
					...
					if ( len ) {
						...
						***REMOVED***
						for ( index = 0; index < len; index++ ) {
							...
							***REMOVED***
						***REMOVED***
						if ( ( c.debug || wo.math_debug ) && console.groupEnd ) { console.groupEnd(); ***REMOVED***
					***REMOVED***
				***REMOVED***);
				if ( ( c.debug || wo.math_debug ) && console.groupEnd ) { console.groupEnd(); ***REMOVED***
				return changed;
	...
	ts.formatMask = function( mask, val, tmpPrefix, tmpSuffix ) {
		...
		***REMOVED***
		...
		var isNegative, result, decimal, group, posLeadZero, posTrailZero, posSeparator, part, szSep,
			...
			prefix = tmp;
		if ( tmpPrefix ) {
			if ( /\{content\***REMOVED***/.test( tmpPrefix || '' ) ) {
				prefix = ( tmpPrefix || '' ).replace( /\{content\***REMOVED***/g, tmp || '' );
			***REMOVED*** else {
		...
		***REMOVED***
		...
		suffix = tmp;
		if ( tmpSuffix ) {
			if ( /\{content\***REMOVED***/.test( tmpSuffix || '' ) ) {
				suffix = ( tmpSuffix || '' ).replace( /\{content\***REMOVED***/g, tmp || '' );
			***REMOVED*** else {
	...
	***REMOVED***;
	...
	ts.equations = {
		...
		***REMOVED***,
		median : function( arry, c ) {
			...
			if ( len > 1 ) {
				...
				arry.sort( function( a, b ){ return a - b; ***REMOVED*** );
		...
		mode : function( arry ) {
			...
			for ( index = 0; index < arry.length; index++ ) {
				...
				if ( m > maxCount ) {
					modes = [ el ];
					maxCount = m;
				***REMOVED*** else if ( m === maxCount ) {
					modes[ modes.length ] = el;
					maxCount = m;
				***REMOVED***
			***REMOVED***
			// returns arry of modes if there is a tie
			return modes.sort( function( a, b ){ return a - b; ***REMOVED*** );
		***REMOVED***,
		max : function(arry) {
		...
		***REMOVED***,
		range: function(arry) {
			var v = arry.sort( function( a, b ){ return a - b; ***REMOVED*** );
		...
		init : function( table, thisWidget, c, wo ) {
			...
			c.$table
				...
				.on( math.events + wo.math_event, function( e ) {
					if ( !this.hasInitialized ) { return; ***REMOVED***
