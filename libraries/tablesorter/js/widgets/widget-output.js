		...
		***REMOVED***,
		...
		processRow: function(c, $rows, isHeader, isJSON) {
			...
			for ( rowIndex = 0; rowIndex < rowsLength; rowIndex++ ) {
				if (!tmpRow[rowIndex]) { tmpRow[rowIndex] = []; ***REMOVED***
				cellIndex = 0;
				...
				for ( indx = 0; indx < cellsLen; indx++ ) {
					...
					if ($cell.filter('[rowspan]').length) {
						...
						for (row = 1; row <= rowspanLen; row++) {
							if (!tmpRow[rowIndex + row]) { tmpRow[rowIndex + row] = []; ***REMOVED***
					...
					***REMOVED***
					...
					while (typeof tmpRow[rowIndex][cellIndex] !== 'undefined') { cellIndex++; ***REMOVED***
		...
		***REMOVED***,
		...
		process : function(c, wo) {
			...
			***REMOVED***
			...
			// /^f/.test(function(){ console.log('test'); ***REMOVED***) is TRUE! (function is converted to a string)
			...
			***REMOVED***
			...
			if ($.isFunction(wo.output_callback)) {
				...
				if ( tmp === false ) {
					return;
				***REMOVED*** else if ( typeof tmp === 'string' ) {
					mydata = tmp;
				***REMOVED***
			***REMOVED***
			...
			***REMOVED***
			...
			if ( gotBlob ) {
				...
				bom = (/^\s*(?:text\/\S*|application\/xml|\S*\/\S*\+xml)\s*;.*charset\s*=\s*utf-8/i.test(wo.output_encoding)) ?
					[ '\ufeff', data ] : [ data ];
				blob = new Blob( bom, { type: wo.output_encoding ***REMOVED*** );
	...
	***REMOVED***;
	...
	ts.addWidget({
		...
		options: {
			...
			output_formatContent  : null, // function(config, widgetOptions, data){ return data.content; ***REMOVED***
			// callback executed when processing completes
			// return true to continue download/output
			// return false to stop delivery & do something else with the data
			output_callback      : function(config, data){ return true; ***REMOVED***,
			// JSON callback executed when a colspan is encountered in the header
			output_callbackJSON  : function($cell, txt, cellIndex) { return txt + '(' + (cellIndex) + ')'; ***REMOVED***,
			...
			output_savePlugin    : null /* function(c, wo, data) {
				var blob = new Blob([data], {type: wo.output_encoding***REMOVED***);
