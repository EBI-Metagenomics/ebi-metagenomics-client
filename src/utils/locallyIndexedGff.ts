...
class GffDB extends Dexie {
  ...
  constructor(name = 'gffdb') {
    ...
    this.version(2).stores({
      contigs: 'contigId, length, *annotations.interpros, *annotations.pfams, *annotations.cogs, *annotations.keggs, *annotations.gos, annotationsPresence.hasInterpros, annotationsPresence.hasPfams, annotationsPresence.hasCogs, annotationsPresence.hasKeggs, annotationsPresence.hasGos',
      //contigId is the primary key since named first - this is the contig integer index so that contigs can be sorted by contig index rather than alphanumerically... contig 312 should be after contig 99. The assembly key needs prepended to this to make a locus tag.
      meta: 'key',
    });
  }
}
...
function createWorker() {
  const worker = new Worker(new URL('./locallyIndexedGff.worker.ts', import.meta.url), { type: 'module' });
...
export type ImportOptions = {
  ...
  batchSize?: number;  // gff lines per indexing batch, may be useful for performance tuning otherwise remove and rely on blockzip size as batch
  onProgress?: (info: { bytes: number; total?: number; percent?: number }) => void;
  onBegin?: () => void;
...
export type TypeAheadAttributes = 'interpros' | 'pfams' | 'keggs' | 'gos' | 'cogs';

export async function getTypeaheadSuggestions(query: string, attribute: TypeAheadAttributes, limit: number = 10): Promise<string[]> {
...
export function importGffToIndexedDB(opts: ImportOptions) {
  ...
  const progressCb = Comlink.proxy((bytes: number, total?: number) => {
    onProgress?.({ bytes, total, percent: total ? (bytes / total) * 100 : undefined });
  });
  ...
  async function bulkUpsertContigAnnotations(
    ...
    for (const { contigId, annotsToAppend, contigName, length } of updates) {
      const acc = merged.get(contigId) ?? {contigName: contigName, length: length, annotations: {}, annotationsPresence: {}};
      for (const [annotType, annotValues] of Object.entries(annotsToAppend)) {
        ...
        (acc as ContigDetails).annotations[annotType] = prev ? prev.concat(annotValues) : annotValues.slice();
      }
    ...
    await db.transaction("rw", db.contigs, async () => {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        if (row) {
          ...
          const next: Contig = { ...row, annotations: { ...(row.annotations || {}) } };
          for (const [field, vals] of Object.entries(appendObj.annotations)) {
            const current = (row.annotations as any)?.[field] as any[] | undefined;
            (next.annotations as any)[field] = uniq([...(current ?? []), ...(vals as any[])]);
          }
  ...
  const onBatch = Comlink.proxy(async (rows: any[]) => {
    ...
    forIn(groupBy(rows, row => row.seqid), (annotations, contigId) => {
      contigsToUpsert.push({
        contigId: contigNameToContigId(contigId),
        contigName: contigId,
        annotsToAppend: {
          interpros: filter(flatMap(annotations, (row => row.indexableAttrValues.interpro)), annot => !!annot),
          pfams: filter(flatMap(annotations, (row => row.indexableAttrValues.pfam)), annot => !!annot),
          cogs: filter(flatMap(annotations, (row => row.indexableAttrValues.cog)), annot => !!annot),
          keggs: filter(flatMap(annotations, (row => row.indexableAttrValues.kegg)), annot => !!annot),
          gos: filter(flatMap(annotations, (row => row.indexableAttrValues.Ontology_term)), annot => !!annot),
        }
      })
    });
    ...
    try {
      ...
      if (clearExisting) {
        await db.transaction('rw', db.meta, db.contigs, async () => {
          ...
          await db.meta.put({ key: 'assemblyAccession', value: assemblyAccession });
          await db.meta.put({ key: 'importStartedAt', value: Date.now() });
        });
      }

      await api.importGff(url, indexUrl, Comlink.proxy(progressCb), Comlink.proxy(onBatch), attrsToIndex, batchSize);
      ...
      if (fastaUrl && fastaFaiUrl && fastaGziUrl) {
        ...
        await db.transaction('rw', db.meta, db.contigs, async () => {
          await db.contigs.bulkUpdate(
            map(contigLengths, (length, contigName) => ({key: contigNameToContigId(contigName), changes: { length}}))
          );
        });
